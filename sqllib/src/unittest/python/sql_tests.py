...
from sqllib import SQLBase, dictobj, Migration, chunked, sql_factory
...
class TestTransactions(unittest.TestCase):
    ...
    def test_select_foreach(self):
        SQL.insert("foo", {"bar": 1, "baz": "aaa"})
        SQL.insert("foo", {"bar": 2, "baz": "bbb"})
        fe = SQL.select_foreach("select * from foo order by bar")
        self.assertEquals(next(fe), {"bar": 1, "baz": "aaa"})
        self.assertEquals(next(fe), {"bar": 2, "baz": "bbb"})
    ...
    def test_select_0or1(self):
        ...
        self.assertEquals(SQL.select_0or1("select * from foo where bar=3"), None)
